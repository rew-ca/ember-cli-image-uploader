...
export function imageTransform(params, hash) {
  ...
  if (hash.url) {
    ...
    return new SafeString('<img src="%@" %@ />'.fmt(url, attributes.join(' ')));
  }
  ...
  return new SafeString('<img src="https://placehold.it/%@&text=%@" %@ />'.fmt(dimensions.join('x'), hash.placeholder, attributes.join(' ')));
}
